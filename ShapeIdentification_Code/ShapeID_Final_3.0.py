    ...
    if (len(approx) > 2):
        ...
        # where shapes are defined as well determing the location of shape
            if len(approx) == 4:
                ...
                final_x = rows(x, y)
                list_of_pieces('Rook', convert_x_to_letter(final_x), columns(x, y))
                ...
                shape = cv2.putText(img_75, 'Bishop/' + str(xSquare) + ", " + str(ySquare), (x, y),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (124, 252, 0), 2)
                final_x = rows(x, y)
                list_of_pieces('Bishop', convert_x_to_letter(final_x), columns(x, y))

            elif len(approx) == 6:
               shape = cv2.putText(img_75, 'Knight/' + str(xSquare) + ", " + str(ySquare), (x, y),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (124, 252, 0), 2)
               final_x= rows(x, y)
               list_of_pieces('Knight', convert_x_to_letter(final_x), columns(x, y))

            elif len(approx) == 7:
                shape = cv2.putText(img_75, 'Pawn/' + str(xSquare) + ", " + str(ySquare), (x, y),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (124, 252, 0), 2)
                final_x = rows(x, y)
                list_of_pieces('Pawn', convert_x_to_letter(final_x), columns(x, y))

            elif len(approx) == 8:
                shape = cv2.putText(img_75, 'Queen/' + str(xSquare) + ", " + str(ySquare), (x, y),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (124, 252, 0), 2)
                final_x = rows(x, y)
                list_of_pieces('Queen', convert_x_to_letter(final_x), columns(x, y))

            elif len(approx) == 9:
                shape = cv2.putText(img_75, 'King/' + str(xSquare) + ", " + str(ySquare), (x, y),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (124, 252, 0), 2)
                final_x = rows(x, y)
                list_of_pieces('king', convert_x_to_letter(final_x), columns(x, y))
