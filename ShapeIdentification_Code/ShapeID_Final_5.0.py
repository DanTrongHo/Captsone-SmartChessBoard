...
image = cv2.imread('Validation10.jpg')
# out2 = skimage.exposure.rescale_intensity(img, in_range=(64, 192), out_range=(0, 255))
# cv2.imwrite('FinalBullshitPic3_192.jpg', out2)
...
red_lower = np.array([150, 100, 50])
red_upper = np.array([400, 350, 250])

# Defining mask for detecting color
mask = cv2.inRange(hsv, red_lower, red_upper)


cv2.imwrite("Mask.jpg", mask) #output mask file

new_red = cv2.resize(image, (1944, 1944))
new_mask = cv2.resize(mask, (1944, 1944))

cv2.imshow("Mask", new_mask)
cv2.imshow("Image", new_red)
...
#img2 = cv2.imread('FinalBullshitPic3_192.jpg')
...
#If statement for calculating total shape length
    if (len(approx) > 2):
        ...
        # are ignored
        if distance < 1000 and distance > 5:
        # where shapes are defined as well determing the location of shape
            if len(approx) == 4:
               ...
               list_of_pieces('Knight', str(convert_x_to_letter(final_x)) + str(rows(x, y)))
                ...
                #shape = cv2.putText(img_75, 'Inval' + str(xSquare) + ", " + str(ySquare), (x, y),
                #            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (124, 252, 0), 2)
                shape =  cv2.putText(img_75, "HERE"+str(len(approx)), (x, y+15),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (124, 252, 0), 2)
        else:
            shape = cv2.putText(img, 'I' , (x, y),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (124, 252, 0), 2)
