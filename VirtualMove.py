...
GraveList= ["s5","s6","s7","s8","a9","b9","c9","d9","e9","f9","g9","h9","t8","t7","t6","t5"]
...
##Checks if movement is a capture
capcheck = board.is_capture(move)


############################################################################################
if capcheck :  

    gravespace=GraveList[gravenum]
    ...
    for i in range(secondX-1):
        kit.stepper2.onestep(direction=stepper.BACKWARD,style=stepper.DOUBLE)
        
    for i in range(secondY-1):
        kit.stepper1.onestep(style=stepper.DOUBLE)
        
    magnet.on()
    gravefind=pathfinding(secondX,secondY,graveX,graveY)
        
    if gravefind[0] < 0 : #DOWN MOVEMENT
        for i in range(abs(gravefind[0])-1):
            kit.stepper1.onestep(direction=stepper.BACKWARD, style=stepper.DOUBLE)
    ...
    if gravefind[1] < 0 : #LEFT MOVEMENT
        for i in range(abs(gravefind[1])-1):
            kit.stepper2.onestep(style=stepper.DOUBLE)
    ...
    if gravefind[2] < 0 : #DOWN MOVEMENT
        for i in range(abs(gravefind[2])-1):
            kit.stepper1.onestep(direction=stepper.BACKWARD, style=stepper.DOUBLE)
    ...
    if gravefind[3] < 0 : #LEFT MOVEMENT
        for i in range(abs(gravefind[3])-1):
            kit.stepper2.onestep(style=stepper.DOUBLE)
